...
namespace SB3Utility
{
	public static partial class Utility
	{
		...
		public static class TGA
		{
			...
			public static Texture2D ToImage(ImportedTexture tex, ImageLoadInformation loadInfo, out byte pixelDepth)
			{
				...
				using (Stream stream = new MemoryStream(tex.Data))
				{
					...
					if (compressed)
					{
						data = Uncompress(stream, width, height, pixelDepth);
					}
					else
					{
						BinaryReader reader = new BinaryReader(stream);
					...
					{
						Report.ReportLog("Warning! Horizontal flipping of TGAs is not supported.");
					}
				...
				byte[] header = DDS.CreateHeader(width, height, 0, pixelDepth, 0, false, pixelDepth == 32 ? DDS.UnityCompatibleFormat.ARGB32 : DDS.UnityCompatibleFormat.RGB24);
			...
			public static byte[] Uncompress(Stream stream, ushort width, ushort height, byte bpp)
			{
				BinaryReader reader = new BinaryReader(stream);
				...
				byte[] uncompressedTGA = new byte[total];
				using (BinaryWriter writer = new BinaryWriter(new MemoryStream(uncompressedTGA)))
				{
					while (writer.BaseStream.Position < total)
					{
						byte packetHdr = reader.ReadByte();
						int packetType = packetHdr & (1 << 7);

						if (packetType != 0)
						{
							byte[] packet = reader.ReadBytes(bytesPerPixel);
		...
		public static class DDS
		{
			...
			public const uint DDS_RESOURCE_MISC_TEXTURECUBE = 0x04;
			...
			public static byte[] CreateHeader(int width, int height, int depth, int rgbBitCount, int mipMaps, bool cubemap, UnityCompatibleFormat format)
			{
				...
				(
					...
					(depth > 0 ? DDS_HEADER_FLAGS.DDSD_DEPTH | (DDS_HEADER_FLAGS)DDS_CAPS2.DDSCAPS2_VOLUME : 0)
				);
				...
				header[27] = (uint)(DDS_CAPS.DDSCAPS_TEXTURE | (mipMaps > 0 ? DDS_CAPS.DDSCAPS_COMPLEX | DDS_CAPS.DDSCAPS_MIPMAP : 0) | (depth > 0 ? DDS_CAPS.DDSCAPS_COMPLEX : 0));
				if (cubemap)
				{
				...
				if (depth > 0)
				{
					header[28] = (uint)DDS_CAPS2.DDSCAPS2_VOLUME;
				}
					...
					|| */ format == UnityCompatibleFormat.BC7)
				{
					...
					if (cubemap)
					{
						header[34] = DDS_RESOURCE_MISC_TEXTURECUBE;
					}
					header[35] = 1;
				}
/*				if (depth > 0)
				{
					header[33] = (uint)ResourceDimension.Texture3D;
					header[35] = 1;
				}*/
			...
			public static int GetMipmapCount(int width, int height, UnityCompatibleFormat format, int dataLengthPerImage)
			{
				...
				int mipmaps = 0;
				int len = 0;
				for (int size; len < dataLengthPerImage || dataLengthPerImage == -1; len += size)
				{
					...
					if (dataLengthPerImage == -1 && width == 1 && height == 1)
					{
						return mipmaps + 1;
					}
				...
				if (dataLengthPerImage >= 0 && len != dataLengthPerImage)
				{
					mipmaps--;
			...
			public static int GetStandardMipmapCount(int width, int height, UnityCompatibleFormat format)
			{
				return GetMipmapCount(width, height, format, -1);
			}
			...
			public static bool Flip(byte[] headerBuffer, Stream input, Stream output, bool writeHeader, bool noMipmaps)
			{
				...
				if (writeHeader)
				{
					...
					(
						...
						DDS_HEADER_FLAGS.DDSD_WIDTH |
						DDS_HEADER_FLAGS.DDSD_PIXELFORMAT |
						(fourCC == FOURCC_DXT1 || fourCC == FOURCC_DXT5 || fourCC == (uint)Format.BC7_UNorm || fourCC == (uint)Format.BC7_UNorm_SRGB ? DDS_HEADER_FLAGS.DDSD_LINEARSIZE : 0) |
						(mipmaps > 0 ? DDS_HEADER_FLAGS.DDSD_MIPMAPCOUNT : 0) |
						(depth > 0 ? DDS_HEADER_FLAGS.DDSD_DEPTH | (DDS_HEADER_FLAGS)DDS_CAPS2.DDSCAPS2_VOLUME : 0)
					);
					header[5] = (header[2] & (uint)DDS_HEADER_FLAGS.DDSD_LINEARSIZE) != 0 ? (uint)(Math.Max(1, ((width + 3) / 4)) * (fourCC == FOURCC_DXT1 ? 2 : 4)/*block_size*/ * height) : 0;
			...
			public static bool DX_Flip(SlimDX.Direct3D11.Device dev, byte[] headerBuffer, Stream input, Stream output)
			{
				uint[] header;
				using (BinaryReader reader = new BinaryReader(new MemoryStream(headerBuffer)))
				{
				...
				int width = (int)header[4];
				int depth = (int)header[6];
				int mipmaps = (int)header[7] > 0 ? (int)header[7] : 1;
				bool cubemap = header[28] == (uint)(DDS_CAPS2.DDSCAPS2_CUBEMAP | DDS_CAPS2.DDS_CUBEMAP_ALLFACES);
				...
				input.Position = 0;
				Texture2D src = Texture2D.FromStream(dev, input, (int)input.Length, imgInfo);
				using (Stream unpacked = new MemoryStream())
				{
					Texture2D.ToStream(dev.ImmediateContext, src, ImageFileFormat.Dds, unpacked);
					src.Dispose();

					byte[] unpackedHeader = new byte[128 + (depth > 0 ? 20 : 0)];
					unpacked.Position = 0;
					unpacked.Read(unpackedHeader, 0, unpackedHeader.Length);
					using (Stream flipped = new MemoryStream())
					{
						flipped.Write(unpackedHeader, 0, unpackedHeader.Length);
						int divSize = 1;
						int blockBytes = 4;
						int rowSize = Math.Max(divSize, (width + divSize - 1)) / divSize * blockBytes;
						int startLayer = 0;
						byte[] row = new byte[rowSize];
						int layerEnd = mipmaps;
						int layerWidth = width;
						int layerHeight = height;
						for (int imgIdx = 0; imgIdx < (cubemap ? 6 : 1) && unpacked.Position < unpacked.Length; imgIdx++)
						{
							for (int i = startLayer; i < layerEnd; i++)
							{
								int blockWidth = Math.Max(divSize, (layerWidth + divSize - 1)) / divSize;
								int blockHeight = Math.Max(divSize, (layerHeight + divSize - 1)) / divSize;
								int blocks = blockWidth * blockHeight;
								int mipmapSize = blocks * blockBytes;
								rowSize = blockWidth * blockBytes;
								long next = unpacked.Position + mipmapSize;
								for (int blockRow = 1; blockRow <= blockHeight; blockRow++)
								{
									unpacked.Position = next - blockWidth * blockRow * blockBytes;
									int read = unpacked.Read(row, 0, rowSize);
									if (read < rowSize)
									{
										Report.ReportLog("Flip failed - texture incomplete");
										return false;
									}
									flipped.Write(row, 0, rowSize);
								}
								...
								unpacked.Position = next;
							}
						}

						imgInfo.Format = Format.BC7_UNorm;
						flipped.Position = 0;
						src = Texture2D.FromStream(dev, flipped, (int)flipped.Length, imgInfo);
					}
				}
				output.Position = 0;
				Texture2D.ToStream(dev.ImmediateContext, src, ImageFileFormat.Dds, output);
				src.Dispose();
				return true;
			}
							...
							: fourCC == FOURCC_DXT5 ? UnityCompatibleFormat.DXT5
								: fourCC == (uint)SlimDX.Direct3D9.Format.A32B32G32R32F ? UnityCompatibleFormat.RGBAFloat
									: fourCC == (uint)SlimDX.Direct3D9.Format.A16B16G16R16F ? UnityCompatibleFormat.RGBAHalf
										: fourCC == (uint)SlimDX.DXGI.Format.BC7_UNorm || fourCC == (uint)SlimDX.DXGI.Format.BC7_UNorm_SRGB ? UnityCompatibleFormat.BC7
				...
				if (originalWidth > maxDim || originalHeight > maxDim || cubemap)
				{
					...
					else
					{
						...
						for (int i = 0; i < (cubemap ? 6 : 1); i++)
						{
							...
							if (i == 0)
							{
								byte[] header = DDS.CreateHeader(width, height * (cubemap ? 6 : 1), 0, BPP, 0, false, format);
		...
		public class SoundLib
		{
			...
			public void GetInfo(byte[] soundBuf, out int channels, out float frequency, out int bits, out uint milliseconds)
			{
				channels = 0;
				frequency = 0;
				bits = 0;
				...
				if (result == FMOD.RESULT.OK)
				{
					if (sound != null)
					{
						...
						result = s.getLength(out milliseconds, FMOD.TIMEUNIT.MS);
						ERRCHECK(result, "sound.getLength");
						FMOD.SOUND_TYPE t;
						FMOD.SOUND_FORMAT f;
						result = s.getFormat(out t, out f, out channels, out bits);
						ERRCHECK(result, "sound.getFormat");
						int priority;
						s.getDefaults(out frequency, out priority);
